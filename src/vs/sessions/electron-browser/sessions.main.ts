...
export class SessionsMain extends Disposable {
	...
	private async initServices(): Promise<{ serviceCollection: ServiceCollection; logService: ILogService; storageService: NativeWorkbenchStorageService; configurationService: ConfigurationService }> {
		...
		// Policies
		const policyChannel = this._register(new PolicyChannelClient(this.configuration.policiesData ?? {}, mainProcessService.getChannel('policy')));
		...
		serviceCollection.set(IPolicyService, policyChannel);
		...
		const [configurationService, storageService] = await Promise.all([
			this.createConfigurationService(workspaceContextService, userDataProfileService, uriIdentityService, fileService, logService, policyChannel, environmentService).then(configurationService => {
