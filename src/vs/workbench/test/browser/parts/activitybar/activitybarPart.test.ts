...
suite('ActivitybarPart', () => {
	...
	function createActivitybarPart(compact: boolean, floatingPanelsEnabled = false): { part: ActivitybarPart; configService: TestConfigurationService; layoutService: TestFloatingPanelsLayoutService } {
		...
		const stubContextKeyService = {
			onDidChangeContext: Event.None,
			bufferChangeEvents: () => {},
			createKey: () => ({ set: () => {}, reset: () => {}, get: () => undefined }),
			contextMatchesRules: () => false,
			getContextKeyValue: () => undefined,
			createScoped: () => stubContextKeyService,
			createOverlay: () => stubContextKeyService,
			getContext: () => ({ getValue: () => undefined }),
			updateParent: () => {},
			dispose: () => {}
		} as unknown as any;
